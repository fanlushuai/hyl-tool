...
for v in y:
    ...
    for d in dic:
        if d["name"] == v.value:
            print("存在追加", v.value, count)
            d["count"] = d["count"] + count
            has = 1
            break

    if has != 1:
        print("不存在新建", v.value, count)

        dic.append({"name": v.value, "count": count})
...
def sort_criteria(item):
    # 自定义排序函数的规则
    return item["count"]  # 返回列表中每个元素的第二个元素作为排序依据
...
sumCount = 0
for d in dic:
    ws.range(i, columns + 2).value = [d["name"], str(d["count"]), ""]
    sumCount += d["count"]
    i += 1
